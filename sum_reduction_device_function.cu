...
__device__ void wrapReduce(volatile int* shmem_ptr, int t) {
...
__global__ void sum_reduction(int *v, int *v_r) {
    ...
    // Vector now 2x as long as number of threads, so scale 1
    int i = blockIdx.x * (blockDim.x * 2) + threadIdx;
    ...
    // Stop early (call device fcuntion instead )
    ...
    // Let the thread 0 for this block write it's result main memory
